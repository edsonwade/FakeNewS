...
app.config['MYSQL_PASSWORD'] = 'Janina1.'
...
def chamar_API_analise_texto(titulo):
    subscription_key = 'fa8f492baff84dfca7de15d8a18ad31a'
...
def chamar_API_noticias(titulo):
    ...
    webpage = requests.get("https://newsapi.org/v2/everything?q="+ titulo +"&apiKey=32d3dbc4eb6040d39a4ce433577391cc", params=None)
...
def guardar_noticia(total_resultados, noticia, texto_noticia, media_plagio, noticias_aceites):
    #Criar Query
    cur = mysql.connection.cursor()
    cur.execute("INSERT INTO noticias(fonte, autor, titulo, descricao, noticia, urlnoticia, urlimagem, datapublicacao, totalresultados, mediaplagio, noticiasaceites) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (noticia.get('source')["name"], noticia.get('author'), noticia.get('title'), noticia.get('description'), texto_noticia, noticia.get('url'), noticia.get('urlToImage'), noticia.get('%Y-%m-%dT%H:%i:%sZ'), total_resultados, media_plagio, noticias_aceites))
...
def tratar_dados(noticia):
    ...
    verdade = float('%0.2f' % (((edata*0.3) + (noticias_aceites*0.15) + (noticia['mediaplagio']*0.133) + (efonte*0.183) + (eautor*0.183) + (eimagem*0.05))*100))
    return verdade
...
def register():
    ...
    if request.method == 'POST' and form.validate():
        ...
        #Criar Query
        cur = mysql.connection.cursor()
        cur.execute("INSERT INTO users(nome, email, password) VALUES(%s, %s, %s)",
                    (name, email, password))
        #Inserir na base de Dados
        mysql.connection.commit()
        #Fechar ligacao a base de dados
